...
-(void)layoutSubviews{
    ...
    if (_isCircleImage) {
        [self drawCircleForImage];
    }

    
}

-(void) drawCircleForImage{
    ...
    //create circle path
    CGFloat radiusPos = MIN(CGRectGetMidX(self.bounds) - 1, CGRectGetMidY(self.bounds)-1);
    //    CGFloat radiusPos      = MIN(CGRectGetMidX(self.bounds) - 1, CGRectGetMidY(self.bounds)-1);
    
    UIBezierPath *circlePath = [UIBezierPath bezierPathWithArcCenter:arcCenter radius:radiusPos startAngle:0.0 endAngle:M_PI *2 clockwise:YES];
    //    let circlePath = UIBezierPath(arcCenter: arcCenter,
    //                                  radius: CGFloat(radiusPos),
    //                                  startAngle: 0.0,
...
- (void)setImage:(UIImage *)image
{
    ...
    if(_displayRealSize){
        if(super.image != nil){
            CGSize imageSize = self.imageScale;
//            _heightConstraint = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem: nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:imageSize.height * super.image.size.height ];

            if (_delegateConstraint) {
                [_delegateConstraint willChangeConstraintHeigh:imageSize.height * super.image.size.height];
            }
//            [self addConstraint:_heightConstraint];
        }
